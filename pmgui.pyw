...
#######################################################
prg_tittle = ''' Password Manager -tkinter- v. 3.0''' 
#######################################################
# author: Jorge Monti
...
| Change URL_IPport|  > Fill Service name and URL_IPport fields to change the URL.\n
...
URL_IPport: Web url or IP:port data necessary to locate the service.
    ex. https://www.reedit.com/, or 192.168.3.3:22\n
Domain: MS-AD Domain, or Cloud Tenant, or DB Instance, etc.
    Third field that define domain to connect.
'''
...
class ShowDF(tk.Frame):
    ...
    def __init__(self, df):
        ...
        f.grid(row=11, columnspan=4, pady=10, sticky='nsew')
...
class PwdMgr(tk.Tk):
    ...
    def read_csd(self):
        try:
            ...
            self.pmt = pmc.PmTable(self.__pph, cipe, kipe)
        except Exception as e:
            self.__ext_prg(f'Critical Error getting Table: {e}')
        else:
            # get columns names as independent vars
            self.s, self.u, self.p, self.r, self.d, self.dt, self.n, self.np, self.nd = self.pmt.getcols()
    ...
    def main_widgets(self):
        ...
        self.sl, self.ul, self.pl, self.rl = f'{self.s}:', f'{self.u}:', f'{self.p}:', f'{self.r}:'
        self.dl, self.nl, self.npl = f'{self.d}:', f'{self.n}:', f'{self.np}:'      # l: per label
        ...
        self.dom_entry = self.__mk_item(self.dl, 7)
        self.nts_entry = self.__mk_item(self.nl, 8)
        self.nxt_pwd_entry = self.__mk_item(self.npl, 9)
...
{self.rl}, {self.dl}, {self.nl} and {self.npl} are optional.''')

    def get_pwd(self):              # '2': ('Get Password', self.getpwd)
        src = self.src_entry.get()
        try:
            pwd, nxt_pwd = self.pmt.getpwd(src)
            msb.showinfo(self.pl, f'{pwd}\n{nxt_pwd}')
        except Exception as e:
            msb.showwarning('ERROR', f"Can't get password for Service '{src}'\n{e}")

    def get_tbl(self):                  # '3': ('Get Table', self.gettbl)
        try:
            ShowDF(self.pmt.gettbl())
        except Exception as e:
            msb.showwarning('ERROR', f"Can't show the table\n{e}")
        
    def get_usr(self):                  # '4': ('Get User', self.getusr)
        src = self.src_entry.get()
        try:
            msb.showinfo(self.ul, self.pmt.getusr(src))
        except Exception as e:
            msb.showwarning('ERROR', f"Can't get user for Service '{src}'\n{e}")

    def chg_pwd(self):                  # '5': ('Change Password', self.chgpwd)
        src = self.src_entry.get()
        ...
        if src and n_pwd and n_pw2:
            try:
                assert n_pwd == n_pw2
                self.__chg_val(src, n_pwd, self.pmt.chgpwd, 'Password Successfully Changed!')
            except AssertionError as e:
                msb.showwarning('Error', f"Passwords don't match {e}")
            except Exception as e:
    ...
    def chg_url(self):                  # '6': ('Change URL', self.chg_url)
        ...
        if src and n_url:
            self.__chg_val(src, n_url, self.pmt.chgurl, 'URL Successfully Changed!')
        else:
    ...
    def updt_nts(self):                 # '7': ('Update Notes', self.updt_nts)
        ...
        if src and n_nt:
            self.__chg_val(src, n_nt, self.pmt.updtnts, 'Notes Successfully Updated!')
        else:
    ...
    def set_nxt_pwd(self):              # '8': ('Set Nxt Pwd', self.set_nxt_pwd)       
        ...
        if src and nxt_pwd:
            try:
                ...
                self.__chg_val(src, nxt_pwd, self.pmt.setnxtpwd,
    ...
    def src_srch(self):                 # '9': ('Service Search', self.src_srch)
        ...
        if part_s:
            ShowDF(self.pmt.srcsrch(part_s))
        else:
            ShowDF(self.pmt.get_empty_df())

    def del_src(self):                  # 'A': ('Delete Service', self.delsrc)
        src = self.src_entry.get()
        try:
            self.pmt.delsrc(src)
    ...
    def table_by_src(self):             # 'B': ('Table by Service', self.tblsrc)
        ShowDF(self.pmt.tblsrc())

    def tbl_icase(self):                # 'C': ('Tbl Ignoring Case', self.tblicase)
        ShowDF(self.pmt.tblicase())

    def f_monti(self):                  # 'D': ('Full Monti', self.fmonti)
        pph3 = self.pph_entry.get()
        self.pph_entry.delete(0, tk.END)
        if pph3 == self.__pph:
            ShowDF(self.pmt.fmonti())
        else:
            ShowDF(self.pmt.get_empty_df())
    ...
    def clr_data(self):                  # cmd[13]
        ShowDF(self.pmt.get_empty_df())
        for e in (self.src_entry, self.usr_entry, self.pwd_entry, self.pw2_entry,
                  self.url_entry, self.dom_entry, self.nts_entry, self.nxt_pwd_entry,
                  self.nxt_pw2_entry):
